		...
		app.measure = {
			...
			start: function () {
				...
				*/
				...
				// improved version of line drawl to make it above the DEM terrains and more visible
				// by Deshen 
				if (!this.geom) {
					var opt = conf.measure.marker;
					this.geom = new THREE.SphereBufferGeometry(opt.radius, 32, 32);
					this.mtl = new THREE.MeshLambertMaterial({color: opt.color, opacity: opt.opacity, transparent: (opt.opacity < 1)});
					opt = conf.measure.line;
					this.lineMtl = new THREE.LineBasicMaterial({
					color: 0xffff00,     // bright yellow for visibility
					linewidth: 3,        // thicker line (works on most WebGL2 contexts)
					transparent: true,   // allow slight glow / anti-alias blend
					opacity: 0.9
					});
					...
					this.lineGroup = new Q3DGroup();
					this.lineGroup.name = "measure line";
					this.lineGroup.position.z += 3;   // lift lines above terrain by ~3 units
