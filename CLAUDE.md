...
**Vendored libraries (do not edit):** `threejs/three.min.js` (r110), `threejs/OrbitControls.js`, `threejs/DeviceOrientationControls.js`, `threejs/ViewHelper.js`, `proj4js/proj4.js`, `fontawesome/*`.

**`Qgis2threejs.js` + `Qgis2threejs.css`:** the viewer framework generated by the QGIS plugin. Exposes the global `Q3D` with `Q3D.application` (camera, renderer, scene, controls, `measure` tool, `cameraAction`) and `Q3D.gui` (popup, layer panel). Treat as third-party.
...
- `Qgis2threejs.js` and `data/index/scene.js` are generated artifacts. Fixes belong in `src/` modules, `custom.css`, or `index.html`, not in the generated files.
...
- The original `custom.js` and `mobile.js` remain in the repo from the initial commit but are no longer loaded by the app. The active code is in `src/`.
